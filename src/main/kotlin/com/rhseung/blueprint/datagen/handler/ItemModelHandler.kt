...
import com.rhseung.blueprint.file.TextureType
...
class ItemModelHandler(
    ...
    data class Texture(var type: String, val id: Loc)

    data class Override(val predicates: MutableMap<Loc, Number>, val model: Model)
    ...
    class Builder {
        ...
        fun model(path: String, lambda: ModelBuilder.() -> Unit): Builder {
            val model = ModelBuilder(Loc(modId, "item/$path"), modId).apply(lambda).build()
    ...
    class ModelBuilder(val id: Loc, val modId: String) {
        ...
        fun build() = Model(id.toIdentifier(), parent, textures)
    }
    ...
    class TextureListBuilder(val modId: String) {
        ...
        operator fun String.unaryPlus() {
            textureList.add(
                Texture(TextureType.LAYER(textureList.count()), Loc(modId, "item/$this"))
            )
        }

        operator fun Pair<String, String>.unaryPlus() {
            textureList.add(Texture(first, Loc(modId, "item/$second")))
        }
        ...
        fun texture(lambda: () -> Pair<String, String>): TextureListBuilder {
            val texture = Texture(lambda().first, Loc(modId, "item/" + lambda().second))
        ...
        fun from(collection: Collection<Texture>, lambda: (Texture) -> Texture): TextureListBuilder {
            collection.forEach { textureList.add(lambda(it)) }
            return this
        }
    ...
    class OverrideBuilder(val modId: String) {
        var predicates = mutableMapOf<Loc, Number>()
        lateinit var model: Model
        ...
        fun predicate(lambda: () -> Pair<String, Number>): OverrideBuilder {
            this.predicates[Loc(modId, lambda().first)] = lambda().second
            return this
        }

        fun model(path: String, lambda: ModelBuilder.() -> Unit): OverrideBuilder {
            this.model = ModelBuilder(Loc(modId, "item/$path"), modId).apply(lambda).build()
            return this
        }
